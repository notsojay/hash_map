...
template<class K, class V>
Pair<K, V>::Pair(const K &key, const V &data):key(key), data(data){}
...
template<class K, class V>
const V& Pair<K, V>::getData() const
{
	return data;
}

template<class K, class V>
void Pair<K, V>::setData(const V& data)
{
	this->data = data;
}
...
HashMap<K, V, H>::HashMap(const size_t& capacity):table(new bktArray(capacity)), entryCount(0){}
...
template<class K, class V, class H>
inline bool HashMap<K, V, H>::empty() const noexcept
{
	return entryCount == 0;
}

template<class K, class V, class H>
inline int HashMap<K, V, H>::size() const noexcept
{
	return entryCount;
}

template<class K, class V, class H>
void HashMap<K, V, H>::nextEntry(iterator& p)
{
	++p.EPos;
}
...
template<class K, class V, class H>
bool HashMap<K, V, H>::beginOfBucket(const iterator& p)
{
...
template<class K, class V, class H>
inline typename HashMap<K, V, H>::iterator HashMap<K, V, H>::end()
{
	return iterator(*table, table->end());
}

template<class K, class V, class H>
class HashMap<K, V, H>::IteratorOfHashMap
{
public:
...
template<class K, class V, class H>
const typename HashMap<K, V, H>::entry& HashMap<K, V, H>::iterator::operator*() const
{
	return *EPos;
}
...
template<class K, class V, class H>
typename HashMap<K, V, H>::iterator& HashMap<K, V, H>::iterator::operator=(const iterator& p)
{
	delete BArr;
	BArr = p.BArr;
	BPos = p.BPos;
